...
function startBroker(behavior = "review-ok") {
  ...
  // This fork's broker authenticates shutdown against a per-instance token and
  // refuses to start without one.
  const instanceToken = `subscriptions-test-${process.pid}-${Math.random().toString(36).slice(2)}`;
  const child = spawn(
    process.execPath,
    [
      BROKER,
