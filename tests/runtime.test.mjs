...
test("task --ephemeral rejects --resume-thread", () => {
  // This fork's third resume form. It resumes by thread id, which an ephemeral run never
  // produces, so the same refusal has to cover it — upstream's check knows only --resume-last.
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeCodex(binDir);
  initGitRepo(repo);

  const result = run("node", [SCRIPT, "task", "--ephemeral", "--resume-thread", "thr_1", "follow up"], {
    cwd: repo,
    env: buildEnv(binDir)
  });

  assert.equal(result.status, 1);
  assert.match(result.stderr, /--ephemeral cannot be combined with --resume\/--resume-last\/--resume-thread/);
});
